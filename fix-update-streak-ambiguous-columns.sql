-- Fix ambiguous column reference errors in update_user_chat_streak function
-- Error: column reference "longest_streak" is ambiguous
...
BEGIN
  ...
  SELECT
    COALESCE(ucs.longest_streak, 0),
    COALESCE(ucs.last_milestone_celebrated, 0)
  ...
  -- Return results using explicit variables to avoid ambiguity
  RETURN QUERY SELECT
    v_current_streak,
    v_longest_streak,
    v_is_new_milestone,
