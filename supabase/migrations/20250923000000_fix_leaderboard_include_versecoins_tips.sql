-- Fix tip leaderboard to include both Stripe tips and VerseCoins tips with proper unit handling
...
BEGIN
  ...
  SELECT
    ...
    at.tip_count::integer,
    ROW_NUMBER() OVER (ORDER BY at.total_amount_cents DESC, at.tip_count DESC)::integer as rank
  FROM aggregated_tips at
  ORDER BY at.total_amount_cents DESC, at.tip_count DESC
  LIMIT p_limit;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Also update the user position function to include VerseCoins tips
...
BEGIN
  ...
  ranked_tips AS (
    SELECT
      ...
      total_amount_cents,
      tip_count,
      ROW_NUMBER() OVER (ORDER BY total_amount_cents DESC, tip_count DESC) as rank
