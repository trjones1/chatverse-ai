...
-- Drop the existing policy that requires auth.uid()
...
-- Create new policies that allow anonymous access through service role
-- but still protect authenticated users' data
CREATE POLICY "Service can manage chat usage" ON daily_chat_usage
  FOR ALL USING (true);

-- Note: With RLS enabled and only the service policy, only the service role
-- (used by our API) can read/write chat usage data. This protects both
-- authenticated and anonymous user data while allowing the API to function.
